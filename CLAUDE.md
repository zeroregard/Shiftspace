...
No scope parenthetical — this project ships a single product (a VSCode extension), so scoping adds noise without information. Write `fix: ...` not `fix(vscode-ext): ...`.
...
| -------------------------------------------------------------------------------- | --------------------------------------------------------- |
| `fix(vscode-ext): wire onOpenExternalUrl in the sidebar so PR/ticket links open` | `fix: links in sidebar now open correctly in the browser` |
| `feat(renderer): add DiffPopover component to FileNode`                          | `feat: show inline diff preview on file hover`            |
| `fix(renderer-core): update Zustand selector in useWorktreeStore`                | `fix: worktree panel no longer re-renders unnecessarily`  |
| `refactor(renderer): extract UnifiedHeader from WorktreeNode`                    | `refactor: simplify worktree header rendering`            |
