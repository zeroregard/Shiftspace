...
export interface InsightRunnerOpts {
  /** Optional function to check if an insight is enabled. Defaults to always true. */
  isInsightEnabled?: (pluginId: string) => boolean;
  /** Optional function to get settings for an insight. Defaults to plugin defaults. */
  getInsightSettings?: (
    pluginId: string,
...
export class InsightRunner {
  ...
  private isInsightEnabled: (pluginId: string) => boolean;
  private getInsightSettings: (
    pluginId: string,
    defaults: Record<string, unknown>
  ) => Record<string, unknown>;

  constructor(opts?: InsightRunnerOpts) {
    this.isInsightEnabled = opts?.isInsightEnabled ?? (() => true);
    this.getInsightSettings = opts?.getInsightSettings ?? ((_id, defaults) => defaults);
  }
  ...
  async analyzeWorktree(opts: {
    ...
    const plugins = insightRegistry.getAll().filter((p) => this.isInsightEnabled(p.id));

    const results = await Promise.allSettled(
      plugins.map(async (plugin) => {
        const settings = this.getInsightSettings(plugin.id, plugin.defaultSettings);
