...
import type { ShiftspaceActionConfig, SmellRule } from '../actions/types';
...
import type { ShiftspaceConfig, CheckResult } from '../actions/types';
...
export interface ConfigProvider {
  readonly config: ShiftspaceConfig;
}
...
export class McpToolHandlers {
  ...
  private handleGetCheckStatus(params: Record<string, unknown>): object {
    ...
    const actions = this.deps.configProvider.config.actions;
  ...
  private async handleRunCheck(params: Record<string, unknown>): Promise<object> {
    ...
    const config = this.deps.configProvider.config.actions.find((a) => a.id === checkId);
  ...
  private async handleRunPipeline(params: Record<string, unknown>): Promise<object> {
    ...
    const pipelines = this.deps.configProvider.config.pipelines;
    ...
    for (const action of this.deps.configProvider.config.actions) {
